...
export async function submitUserData(values: any, endpoint: string, type: number, role: number): Promise<any> {
	const formData = new FormData();
	for (const key of Object.keys(values)) {
		const value = values[key];
		if (value !== null && value !== undefined && value !== '') {
			if (key === 'profile') {
				...
				imgformData.append('path', '/uploads/user_images');
			...
			} else if (key === 'old_profile') {
				formData.append('profile', value);
			} else {
				formData.append(key, value);
			}
		}
